...
public class MlcalcTestNG {
    ...
    @BeforeSuite
    public void preconditions() throws IOException, InterruptedException,  IOException, BiffException {
    ...
    public void testScenario() throws WriteException, InterruptedException {
        ...
        for(int i =1; i <rowCount; i++){
            ...
            //enter mortgage term
            ReusableMethods.sendKeysMethod(driver,"//input[@id='mt']",mTerm,"Mortgage Term");
            //enter interest rate
            ReusableMethods.sendKeysMethod(driver,"//input[@id='ir']",iRate,"Interest Rate");
            //select start month
            ReusableMethods.dropdownByText(driver,"//select[@name='sm']",stMonth,"Start Month");
            //select start year
            ReusableMethods.dropdownByText(driver,"//select[@name='sy']",stYear,"Start Year");
            //click on Calculate
            ReusableMethods.clickOnElement(driver,"//input[@value='Calculate']","Calculate");

            //capture monthly payment
            String monthlyPayment = ReusableMethods.captureText(driver,"//td[@align='center']",0,"Monthly Payment");
